...
def stylize_static_image(inference_config):
    ...
    training_state = torch.load(os.path.join(inference_config["model_binaries_path"], inference_config["model_name"]), map_location)
    ...
    with torch.no_grad():
        if os.path.isdir(inference_config['content_input']):  # do a batch stylization (every image in the directory)
            ...
            utils.save_and_maybe_display_image(inference_config, stylized_img, name="./a.jpg", should_display=inference_config['should_not_display'])
...
if __name__ == "__main__":
    ...
    #
    content_images_path = os.path.join(os.path.dirname(__file__), 'Content')
    ...
    parser.add_argument("--content_input", type=str, help="Content image(s) to stylize", default='w14_1.jpg')
    parser.add_argument("--batch_size", type=int, help="Batch size used only if you set content_input to a directory", default=5)
    parser.add_argument("--img_width", type=int, help="Resize content image to this width", default=250)
    parser.add_argument("--img_height", type=int, help="Resize content image to this height", default=300)
    parser.add_argument("--model_name", type=str, help="Model binary to use for stylization", default='a1_mosaic.pth')

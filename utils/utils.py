...
#     print(f"Successfully loaded repository at {path_to_repo}")
# except InvalidGitRepositoryError:
#     print(f"Error: {path_to_repo} is not a valid Git repository.")
#     # Optionally, initialize the repository if it's not valid
#     if not os.path.exists(path_to_repo):
#         os.makedirs(path_to_repo)
#         print(os.makedirs(path_to_repo))
#     repo = Repo.init(path_to_repo)
#     print(f"Initialized a new Git repository at {path_to_repo}, {repo}")
...
def save_and_maybe_display_image(inference_config, dump_img, name, should_display=False):
    ...
    if inference_config['redirected_output'] is None:
        ...
        dump_img_name = os.path.basename(name)
        print(f"name1{dump_img_name}")
    ...
    if inference_config['verbose'] and not os.path.isdir(inference_config['content_input']):
        print(f'Saved image to {dump_dir}.{dump_img_name}')
    #
